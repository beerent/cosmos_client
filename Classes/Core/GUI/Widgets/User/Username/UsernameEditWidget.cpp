...
void UsernameEditWidget::OnExitPressed(UITouchButton::ButtonState state) {
    ...
    const User newUser = User(currentUser.GetDeviceUuid(), m_username, currentUser.GetPasswordHash(), currentUser.GetAccessLevel());
...
void UsernameEditWidget::SetCursorTimer() {
    m_timer.ResetTimer(Timer::TimerType::CURSOR_BLINK_550_MS);
}

void UsernameEditWidget::ResetCursorTimer() {
    m_timer.ResetTimer(Timer::TimerType::CURSOR_BLINK_550_MS);
}

void UsernameEditWidget::UnsetCursorTimer() {
